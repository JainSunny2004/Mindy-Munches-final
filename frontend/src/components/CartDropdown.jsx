...
const CartDropdown = ({ isOpen, onClose, isMobile = false }) => {
  ...
  const handleProceedToCheckout = () => {
    if (!isAuthenticated) {
      ...
      notification.className = 'fixed top-20 right-4 bg-yellow-500 text-white px-4 py-2 rounded-lg shadow-lg z-50 text-sm'
    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          {/* Cart Items */}
          <div className={`${isMobile ? 'max-h-48' : 'max-h-60'} overflow-y-auto bg-white`}>
            <AnimatePresence>
